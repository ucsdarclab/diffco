...
from diffco.Obstacles import ObstacleMotion, LinearMotion
...
def create_plots(robot, obstacles, gt_est, checker):
    ...
    size = [400, 400]
    yy, tt = torch.meshgrid(
        torch.linspace(0, 1, size[1]),
        torch.linspace(0, 1, size[0]))
    grid_points = torch.stack([yy, tt], dim=2)
    unnorm_yytt = robot.unnormalize(grid_points)
    yy, tt = unnorm_yytt[:, :, 0], unnorm_yytt[:, :, 1]
    ...
    score_gt = gt_est(grid_points).reshape(size+[num_class])
    score_pred = checker.rbf_score(grid_points).reshape(size+[num_class])
    
    with sns.axes_style('ticks'):
        # for cat in range(num_class):
        cat = 0
        c_ax = fig.add_subplot(gs[0, cat])
        ...
        color_mesh = c_ax.pcolormesh(
            tt, yy, pred, cmap=cmaps[cat], vmin=-3, vmax=3) #torch.abs(pred).max()
        c_support_points = robot.unnormalize(checker.support_points) #[checker.gains[:, cat] != 0]
        c_ax.scatter(
            c_support_points[:, 1], c_support_points[:, 0], marker='.', c='black', s=1.5)
        # -1.5, -0.75, 0, 0.3
        c_ax.contour(tt, yy, gt, levels=[0], 
            linewidths=1, alpha=0.4, colors='g')
        # c_ax.contour(tt, yy, pred, levels=[0], 
        #     linewidths=0.2, alpha=0.4, colors='r')
        ...
        # c_ax.axis('equal')
        c_ax.set_xlim(0, 25)
        c_ax.set_ylim(robot.limits[0, 0], robot.limits[0, 1])
        c_ax.set_xticks(range(0, 25, 5))
        c_ax.set_xticklabels([f'${str(t)}$' for t in range(0, 25, 5)])
...
def main(checking_method='diffco'):
    ...
    gt_checker = Simple1DDynamicChecker(obstacles=dynamic_obs, robot=robot)

    label_type = 'binary'
    num_class = 1

    T = 20
    tstep = 1.0 # step size of each timestep in seconds
    grad_exploit_steps = 5 # # of how many substeps to take using partial gradient in exploitation
    nu = 5  # number of exploitation
    kai = 100 # number of random exploration
    sigma = 0.03
    seed = 1918
    ...
    checker = MultiDiffCo(obstacles, kernel_func=kernel.TemporalFKKernel(
        fkine, kernel.RQKernel(100), kernel.RQKernel(100), alpha=3), beta=1.0)
    plt.axis('equal')
    sct = plt.scatter(cfgs[:, -1], cfgs[:, 0], c=dists[:, 0]-1, s=1)
    plt.colorbar(sct)
    plt.show()
    checker.train(cfgs[:train_num], labels[:train_num], max_iteration=train_num*2, distance=dists[:train_num])
    plt.scatter(checker.support_points[:, 1], checker.support_points[:, 0], marker='.', c=checker.hypothesis)
    plt.show()
    ...
    def gt_diffco(point):
        return gt_checker.predict(point, distance=True)[1]
    dist_est = gt_diffco
    print(f'MIN_SCORE = {dist_est(cfgs[train_num:]).min():.6f}')
    ...
    for t in range(T):
        if t % 1 == 0:
            ...
            fig_dir = f'figs/temporal_active/{seed}/{checking_method}/{env_name}'
        ...
        # shift time range
        robot.limits[-1] += tstep
        tscale = robot.limits[-1, 1] - robot.limits[-1, 0]
        new_samples = []

        newest_support_points = checker.support_points[checker.support_points[:, -1] > 0.9]
        if len(newest_support_points) > 0:
            ...
            exploit_centers = newest_support_points.detach()[:, None] + delta_ts * pd[:, None]
            exploit_centers = exploit_centers.reshape(-1, robot.dof+1)
            exploit_samples = torch.randn(nu, len(exploit_centers), robot.dof+1, 
                dtype=checker.support_points.dtype) * sigma + exploit_centers
            ...
            new_samples.append(exploit_samples)
            print(f'{len(exploit_samples)} exploitation samples')

        # calculate explore samples
        explore_samples = torch.rand(
            kai, robot.dof+1, dtype=checker.support_points.dtype)
        explore_samples[:, -1] = 1 + \
        ...
        new_labels, new_dists = gt_checker.predict(new_samples, distance=True)
        new_labels = new_labels.reshape(-1, 1)
        new_dists = new_dists.double().reshape(-1, 1)
        ...
        cfgs = torch.cat([new_samples, checker.support_points])
        gains = torch.cat([
            torch.zeros(len(new_samples), checker.num_class, dtype=checker.gains.dtype), 
            checker.gains])  # None #
        # TODO: bug: not calculating true hypothesis for new points
        added_hypothesis = checker.score(new_samples)
        ...
        labels = torch.cat([new_labels, checker.y])
        dists = torch.cat([new_dists, checker.distance])

        checker.train(cfgs, labels, gains=gains, hypothesis=hypothesis, distance=dists, max_iteration=60000, kernel_matrix=kernel_matrix)
    ...
    if not isdir(fig_dir):
        makedirs(fig_dir)
    plt.savefig(join(fig_dir, f'temp1d_{DOF}dof_{env_name}_{checking_method}_{T:02d}'), dpi=600)
