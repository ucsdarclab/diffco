...
def main(
    ...
    if dataset_filepath is None:
        dataset_filepath = autogenerate_dataset(task, random_seed)
...
def autogenerate_dataset(task: str, random_seed: int = None) -> str:
    """Autogenerate a dataset using a default configuration that suits the
    desired task.
    
    Args:
        task (str): The task to complete. Must be 'correlate' or 'compare'.
        random_seed (int): Random seed used to reproduce the same results,
            useful for debugging. Defaults to None.
    ...
    """
    # Define some default configurations for each task
    if task == 'correlate':
        default_dof = 3
        default_num_obstacles = 150
        default_label_type = 'binary'
        default_env_name = '3d_halfnarrow'
        generate_random_cfgs = True
        num_init_points = 8000
    elif task == 'compare':
        default_dof = 2
        default_num_obstacles = 1
        default_label_type = 'binary'
        default_env_name = '1rect'
        generate_random_cfgs = False
        num_init_points = 160000
    else:
        raise ValueError(task)
    
    # Check if the dataset exists already
    output_dir = 'data/autogenerated'
    filename = f'2d_{default_dof}dof_{default_num_obstacles}obs_{default_label_type}_{default_env_name}.pt'
    ...
    if not os.path.exists(dataset_filepath):
        ...
        dataset_generation.main(env_name=default_env_name, folder=output_dir,
            label_type=default_label_type, dof=default_dof, num_init_points=num_init_points,
            random_seed=random_seed, generate_random_cfgs=generate_random_cfgs)
    else:
